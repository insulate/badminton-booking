...
- **Payment settings tests** (`payment-settings.spec.js`) — backup/restore ผ่าน `GET /api/settings` + `PATCH /api/settings/payment` (ไม่ต้อง restore `qrCodeImage` เพราะ PATCH payment ไม่แตะ field นั้น) — cross-page ที่ได้รับผลกระทบหลักคือ **PaymentPage** (`/payment/:bookingId`) ซึ่ง customer ใช้ชำระเงิน
- **Cross-Page test สำหรับ PaymentPage** — ต้องสร้าง booking ชั่วคราวผ่าน API ก่อน navigate เพราะ page ต้องการ `bookingId` จริง; booking สร้างโดย admin default `bookingStatus = 'confirmed'` → PaymentPage แสดง success state แทน payment form → ต้อง `PATCH /:id` เป็น `checked-in` ก่อน (valid transition: `confirmed → checked-in`); cleanup ใช้ `PATCH /:id/cancel` (ไม่มี `DELETE /api/bookings/:id`); ใช้ `playwrightRequest.newContext()` ใน `finally` แทน `request` fixture เพราะ page context อาจถูกปิดก่อนถ้า test timeout
...
- **Player levels settings tests** (`player-levels-settings.spec.js`) — backup ผ่าน `GET /api/settings/player-levels` (public, ไม่ต้องใช้ token), restore ผ่าน `PATCH /api/settings/player-levels` body `{ levels: [...] }` (ต้องใช้ token) — cross-page ที่ได้รับผลกระทบคือ **PlayersPage** (`/admin/players`) filter dropdown และ **PlayerForm** level select modal
- **Settings pages ที่ fetch ข้อมูลจาก API ตอน mount** — `page.goto()` resolve เมื่อ HTML โหลดแต่ก่อน API call เสร็จ (ยังเป็น spinner) → `.count()` และ `evaluateAll()` ที่เรียกทันทีหลัง goto จะได้ 0 / `[]`; Page Object ควร `await saveButton.waitFor()` ใน `goto()` เพื่อรอให้ข้อมูลโหลดก่อน; เช่นเดียวกันหลัง `page.reload()` ต้อง `waitFor(saveButton)` ก่อนเรียก `.count()` หรือ `evaluateAll()` (ต่างจาก `expect(locator).toHaveValue()` ซึ่ง auto-wait ได้เอง)
- **HTML5 `required` บล็อก React custom validation** — input ที่มีทั้ง `required` attribute และ React validate `!name.trim()` พร้อมกัน: `fill('')` จะให้ HTML5 บล็อก form submit ก่อน React handler ทำงาน → toast ของ React จะไม่ปรากฏ; ต้องใช้ whitespace-only `'   '` แทน (ผ่าน HTML5 required เพราะไม่ใช่ empty string แต่ React `trim()` จะตรวจจับได้)
